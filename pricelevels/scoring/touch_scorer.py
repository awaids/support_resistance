...
class TouchScorer(BaseScorer):
    ...
    def __init__(
        self,
        min_candles_between_body_cuts=5,
        diff_perc_from_extreme=0.05,
        min_distance_between_levels=0.1,
        min_trend_percent=0.5,
        diff_perc_for_candle_close=0.05,
        score_for_cut_body=-2,
        score_for_cut_wick=-1,
        score_for_touch_high_low=1,
        score_for_touch_normal=2,
    ):
    ...
    def closeFromExtreme(self, key, min_value, max_value):
        return abs(key - min_value) < (
            min_value * self.DIFF_PERC_FROM_EXTREME / 100.0
        ) or abs(key - max_value) < (max_value * self.DIFF_PERC_FROM_EXTREME / 100)
        ...
        for i, obj in enumerate(levels):
            if isinstance(obj, float):
                ...
                try:
                    ...
                    raise Exception(
                        "`levels` supposed to be a list of floats or list of dicts with `price` key"
                    )
            else:
                raise Exception(
                    "`levels` supposed to be a list of floats or list of dicts with `price` key"
    ...
    def _get_level_score(self, candles, high_low_marks, price):
        ...
        for i in range(len(candles)):
            ...
            if (
                self.cut_body(price, candle)
                and i - last_cut_pos > self.MIN_DIFF_FOR_CONSECUTIVE_CUT
            ):
                score += self.score_for_cut_body
                last_cut_pos = i
                events.append(
                    PointEvent(
                        PointEventType.CUT_BODY,
                        candle["OpenTime"],
                        self.score_for_cut_body,
                    )
                )
            # If the wick of the candle cuts through the price, then deduct some score
            elif self.cut_wick(price, candle) and (
                i - last_cut_pos > self.MIN_DIFF_FOR_CONSECUTIVE_CUT
            ):
                score += self.score_for_cut_wick
                last_cut_pos = i
                events.append(
                    PointEvent(
                        PointEventType.CUT_WICK,
                        candle["OpenTime"],
                        self.score_for_cut_body,
                    )
                )
    ...
    def in_down_trend(self, candles, price, start_pos):
        ...
        while pos >= 0:
            if candles["Low"].iat[pos] < price:
                return False
            if candles["Low"].iat[pos] - price > (
                price * self.MIN_PERC_FOR_TREND / 100
            ):
                return True
            pos -= 1
    ...
    def touch_high(self, price, candle):
        ...
        return (
            high <= price
            and abs(high - price) < ltp * self.DIFF_PERC_FOR_CANDLE_CLOSE / 100
        )
    ...
    def touch_low(self, price, candle):
        ...
        return (
            low >= price
            and abs(low - price) < ltp * self.DIFF_PERC_FOR_CANDLE_CLOSE / 100
        )

    def cut_body(self, point, candle):
        return (
            max(candle["Open"], candle["Close"]) > point
            and min(candle["Open"], candle["Close"]) < point
        )

    def cut_wick(self, price, candle):
        return (
            not self.cut_body(price, candle)
            and candle["High"] > price
            and candle["Low"] < price
        )
